...
/**
 * Обработка успешной покупки билетов
 */
function handleSuccessfulPurchase(ticketCount) {
    // Получаем текущего пользователя
    const currentUser = JSON.parse(localStorage.getItem('cf_currentUser'));
    if (!currentUser) return;

    // Обновляем количество билетов
    currentUser.tickets += ticketCount;
    ...
    currentUser.activityLog.push({
        ...
        message: `Покупка ${ticketCount} билетов`,
        reward: `+${ticketCount} билетов`
    });

    // Сохраняем пользователя
    localStorage.setItem('cf_currentUser', JSON.stringify(currentUser));

    // Проверяем реферальные награды
    checkAndApplyReferralRewards(currentUser.id);
...
// Вызывайте эту функцию после успешной покупки билетов
// Например, в обработчике подтверждения оплаты:
document.getElementById('confirmPaymentBtn')?.addEventListener('click', function() {
    const ticketCount = parseInt(document.getElementById('modalTickets').textContent);
    handleSuccessfulPurchase(ticketCount);
    showNotification('Билеты успешно куплены!', 'success');
    closePaymentModal();
});