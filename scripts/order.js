...
function showSuccessAnimation() {
  ...
  checkmark.innerHTML = '<svg viewBox="0 0 52 52" style="width:100px;height:100px"><circle cx="26" cy="26" r="25" fill="none" stroke="#6a5acd" stroke-width="3"/><path fill="none" stroke="#6a5acd" stroke-width="4" d="M14.1 27.2l7.1 7.2 16.7-16.8"/></svg>';

  const message = document.createElement('h2');
  message.textContent = 'Спасибо за заказ!';
  message.style.color = 'var(--primary)';
  message.style.marginTop = '20px';
  message.style.fontSize = '2rem';

  const submessage = document.createElement('p');
  submessage.textContent = 'Мы свяжемся с вами в ближайшее время';
  submessage.style.color = 'var(--text)';
  ...
  // Анимация летающих бокалов
  for (let i = 0; i < 5; i++) {
    setTimeout(() => {
      const glass = document.createElement('div');
      glass.innerHTML = '<img src="images/wine-glass.png" style="width:50px;height:50px;opacity:0.7;">';
      glass.style.position = 'absolute';
      glass.style.top = '50%';
      glass.style.left = '50%';
      glass.style.transform = 'translate(-50%, -50%)';
      glass.style.animation = `fly-${i} 3s ease-in forwards`;
      ...
      style.textContent = `
        @keyframes fly-${i} {
          ...
          100% { transform: translate(${Math.random() * 100 - 50}vw, ${Math.random() * 100 - 150}vh) rotate(360deg); opacity: 0; }
        }
      `;
