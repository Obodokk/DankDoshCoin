document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadDesignContent(design) {
      ...
      // Загрузка шаблона, если он есть
      if (design.template) {
        fabric.Image.fromURL(`images/templates/${design.template.name}`, function(img) {
          const offsetX = (design.template.left - centerX) * scale;
          const offsetY = (design.template.top - centerY) * scale;
    ...
    phoneInput.addEventListener('input', function(e) {
        // Получаем только цифры после +373
        let phoneDigits = this.value.replace(/\D/g, '').replace(/^373/, '');

        // Ограничиваем длину номера (8 цифр без кода страны)
        ...
        // Проверяем валидность (ровно 8 цифр после кода страны)
        if (phoneDigits.length === 8) {
    ...
    // Начальное состояние - невалидное
