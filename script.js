...
function handleCoinClick(e) {
    ...
    // Получаем позицию клика относительно монеты
    ...
    if (isCritical) {
        const critMult = 2 + (gameState.upgrades.criticalPower - 1) * 0.5;
        multiplier *= critMult;
        createClickEffect(clickX, clickY, `CRIT! x${critMult.toFixed(1)}`, true);
        gameState.totalCriticals++;
        gameState.highestCritical = Math.max(gameState.highestCritical, critMult);
    }
    ...
    createClickEffect(clickX, clickY, `+${earned}`, false);
...
function createClickEffect(x, y, text, isCritical) {
    ...
    requestAnimationFrame(() => {
        ...
        effect.style.transform = 'translate(-50%, -50%) scale(1.5) translateY(-80px)';

        // Анимация исчезновения
        setTimeout(() => {
            effect.style.opacity = '0';
            effect.style.transform = 'translate(-50%, -50%) scale(2) translateY(-120px)';
